...
public class AdaptiveTile : Tile {
    ...
    public override void GetTileData(Vector3Int location, ITilemap tilemap, ref TileData tileData)
    {
        ...
        if (index >= 0 && index < m_Sprites.Length)
        {
            ...
            Quaternion rotation = GetRotation((byte)mask);
    ...
    [MenuItem("Assets/Create/AdaptiveTile")]
    public static void CreateAdaptiveTile()
    {
        string path = EditorUtility.SaveFilePanelInProject("Save Adaptive Tile", "New Adaptive Tile", "Asset", "Save Adaptive Tile", "Assets");
        if (path == "")
            return;
        AssetDatabase.CreateAsset(ScriptableObject.CreateInstance<AdaptiveTile>(), path);
    }
#endif
}
